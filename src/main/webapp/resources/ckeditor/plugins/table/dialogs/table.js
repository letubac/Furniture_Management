...
(function () {
  function v (a) { for (var f = 0, n = 0, l = 0, p, e = a.$.rows.length; l < e; l++) { p = a.$.rows[l]; for (var d = f = 0, b, c = p.cells.length; d < c; d++)b = p.cells[d], f += b.colSpan; f > n && (n = f) } return n } function r (a) { return function () { var f = this.getValue(); var f = !!(CKEDITOR.dialog.validate.integer()(f) && f > 0); f || (alert(a), this.select()); return f } } function q (a, f) {
    const n = function (e) { return new CKEDITOR.dom.element(e, a.document) }; const q = a.editable(); const p = a.plugins.dialogadvtab; return {
      title: a.lang.table.title,
      minWidth: 310,
      minHeight: CKEDITOR.env.ie
        ? 310
        : 280,
      onLoad: function () { const e = this; const a = e.getContentElement('advanced', 'advStyles'); if (a)a.on('change', function () { let a = this.getStyle('width', ''); let c = e.getContentElement('info', 'txtWidth'); c && c.setValue(a, !0); a = this.getStyle('height', ''); (c = e.getContentElement('info', 'txtHeight')) && c.setValue(a, !0) }) },
      onShow: function () {
...
'both')) for (h = 0; h < b.$.rows.length; h++)g = new CKEDITOR.dom.element(b.$.rows[h].cells[0]), g.renameNode('th'), g.setAttribute('scope', 'row'); if (this.hasColumnHeaders && f != 'col' && f != 'both') for (k = 0; k < b.$.rows.length; k++)h = new CKEDITOR.dom.element(b.$.rows[k]), h.getParent().getName() == 'tbody' && (g = new CKEDITOR.dom.element(h.$.cells[0]), g.renameNode('td'), g.removeAttribute('scope')); c.txtHeight ? b.setStyle('height', c.txtHeight) : b.removeStyle('height'); c.txtWidth ? b.setStyle('width', c.txtWidth) : b.removeStyle('width')
        ...
        } if (this._.selectedElement) try { e.selectBookmarks(d) } catch (p) {} else a.insertElement(b), setTimeout(function () { const e = new CKEDITOR.dom.element(b.$.rows[0].cells[0]); const c = a.createRange(); c.moveToPosition(e, CKEDITOR.POSITION_AFTER_START); c.select() }, 0)
      },
      contents: [{
        id: 'info',
        ...
        elements: [{
          ...
          children: [{
            ...
            children: [{
              ...
              setup: function (e) { const a = this.getDialog(); a.hasColumnHeaders = !0; for (let b = 0; b < e.$.rows.length; b++) { const c = e.$.rows[b].cells[0]; if (c && c.nodeName.toLowerCase() != 'th') { a.hasColumnHeaders = !1; break } }e.$.tHead !== null ? this.setValue(a.hasColumnHeaders ? 'both' : 'row') : this.setValue(a.hasColumnHeaders ? 'col' : '') },
              commit: l
            }, {
              type: 'text',
              id: 'txtBorder',
            ...
            children: [{
              ...
              children: [{
                ...
                validate: CKEDITOR.dialog.validate.cssLength(a.lang.common.invalidCssLength.replace('%1',
                  a.lang.common.width)),
                onChange: function () { const a = this.getDialog().getContentElement('advanced', 'advStyles'); a && a.updateStyle('width', this.getValue()) },
              ...
              children: [{
                ...
                onChange: function () { const a = this.getDialog().getContentElement('advanced', 'advStyles'); a && a.updateStyle('height', this.getValue()) },
          ...
          children: [{
            ...
            setup: function (a) {
              this.enable(); a = a.getElementsByTag('caption'); if (a.count() > 0) {
                a = a.getItem(0); const d = a.getFirst(CKEDITOR.dom.walker.nodeType(CKEDITOR.NODE_ELEMENT)); d && !d.equals(a.getBogus())
                  ? (this.disable(), this.setValue(a.getText()))
                  : (a = CKEDITOR.tools.trim(a.getText()),
                    this.setValue(a))
              }
            },
            commit: function (e, d) { if (this.isEnabled()) { let b = this.getValue(); let c = d.getElementsByTag('caption'); if (b)c.count() > 0 ? (c = c.getItem(0), c.setHtml('')) : (c = new CKEDITOR.dom.element('caption', a.document), d.append(c, !0)), c.append(new CKEDITOR.dom.text(b, a.document)); else if (c.count() > 0) for (b = c.count() - 1; b >= 0; b--)c.getItem(b).remove() } }
          }, {
            type: 'text',
            id: 'txtSummary',
            bidi: !0,
...
'')
  ...
  } var u = CKEDITOR.tools.cssLength; var l = function (a) { const f = this.id; a.info || (a.info = {}); a.info[f] = this.getValue() }; CKEDITOR.dialog.add('table', function (a) { return q(a, 'table') }); CKEDITOR.dialog.add('tableProperties', function (a) { return q(a, 'tableProperties') })
})()
