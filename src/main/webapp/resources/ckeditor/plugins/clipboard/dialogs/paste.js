...
CKEDITOR.dialog.add('paste', function (c) {
  function k (a) {
    const b = new CKEDITOR.dom.document(a.document); const g = b.getBody(); const d = b.getById('cke_actscrpt'); d && d.remove(); g.setAttribute('contenteditable', !0); g.on(e.mainPasteEvent, function (a) { a = e.initPasteDataTransfer(a); f ? a != f && (f = e.initPasteDataTransfer()) : f = a }); if (CKEDITOR.env.ie && CKEDITOR.env.version < 8)b.getWindow().on('blur', function () { b.$.selection.empty() }); b.on('keydown', function (a) {
...
9:this.changeFocus(1), b = 1
  ...
  } const h = c.lang.clipboard; var e = CKEDITOR.plugins.clipboard; let f; c.on('pasteDialogCommit', function (a) { a.data && c.fire('paste', { type: 'auto', dataValue: a.data.dataValue, method: 'paste', dataTransfer: a.data.dataTransfer || e.initPasteDataTransfer() }) }, null, null, 1E3); return {
    title: h.title,
    minWidth: CKEDITOR.env.ie && CKEDITOR.env.quirks
      ? 370
      : 350,
...
h.securityMsg + '\x3c/div\x3e'
        ...
        html: '',
        focus: function () { const a = this.getInputElement(); const b = a.getFrameDocument().getBody(); !b || b.isReadOnly() ? a.setCustomData('pendingFocus', 1) : b.focus() },
        setup: function () {
...
'})())"'
        ...
        commit: function () {
          ...
          const b = this.getInputElement().getFrameDocument().getBody(); const c = b.getBogus(); let d; c && c.remove(); d = b.getHtml(); setTimeout(function () { a.fire('pasteDialogCommit', { dataValue: d, dataTransfer: f || e.initPasteDataTransfer() }) }, 0)
        }
      }]
    }]
  }
