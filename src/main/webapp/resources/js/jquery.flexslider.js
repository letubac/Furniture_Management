...
(function ($) {
  ...
  $.flexslider = function (el, options) {
    ...
    const namespace = slider.vars.namespace
    const msGesture = window.navigator && window.navigator.msPointerEnabled && window.MSGesture
    const touch = (('ontouchstart' in window) || msGesture || window.DocumentTouch && document instanceof DocumentTouch) && slider.vars.touch
    // depricating this idea, as devices are being released with both of these events
    // eventType = (touch) ? "touchend" : "click",
    const eventType = 'click touchend MSPointerUp keyup'
    let watchedEvent = ''
    let watchedEventClearTimer
    const vertical = slider.vars.direction === 'vertical'
    const reverse = slider.vars.reverse
    const carousel = (slider.vars.itemWidth > 0)
    const fade = slider.vars.animation === 'fade'
    const asNav = slider.vars.asNavFor !== ''
    let methods = {}
    const focused = true
    ...
    methods = {
      ...
      controlNav: {
        ...
        setupPaging: function () {
          const type = (slider.vars.controlNav === 'thumbnails') ? 'control-thumbs' : 'control-paging'
          let j = 1
          let item
          let slide
          ...
          if (slider.pagingCount > 1) {
            for (let i = 0; i < slider.pagingCount; i++) {
              ...
              item = (slider.vars.controlNav === 'thumbnails') ? '<img src="${pageContext.request.contextPath}/' + slide.attr('data-thumb') + '"/>' : '<a>' + j + '</a>'
    ...
    slider.flexAnimate = function (target, pause, override, withSync, fromNav) {
      ...
      if (!slider.animating && (slider.canAdvance(target, fromNav) || override) && slider.is(':visible')) {
        if (asNav && withSync) {
          const master = $(slider.vars.asNavFor).data('flexslider')
        ...
        if (!fade) {
          ...
          let margin; let slideString; let calcNext
    ...
    slider.setProps = function (pos, special, dur) {
      let target = (function () {
        const posCheck = (pos) || ((slider.itemW + slider.vars.itemMargin) * slider.move) * slider.animatingTo
    ...
    slider.doMath = function () {
      ...
      const slideMargin = slider.vars.itemMargin
      const minItems = slider.vars.minItems
      const maxItems = slider.vars.maxItems
